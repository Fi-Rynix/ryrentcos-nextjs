...
export default function VideoDeleteDialog({ video, onClose }: VideoDeleteDialogProps) {
  const [isDeleting, setIsDeleting] = useState(false);
  const [confirmName, setConfirmName] = useState("");

  const handleDelete = async () => {
    if (confirmName !== video.title) return;
  ...
  const isConfirmValid = confirmName === video.title;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
        ...
        <div className="mb-6 text-center">
          <h2 className="text-xl font-bold text-zinc-800">Hapus Video?</h2>
          <p className="mt-2 text-sm text-zinc-500">
            Apakah kamu yakin ingin menghapus video{" "}
            <span className="font-bold text-zinc-700">{video.title}</span>?
        ...
        <div className="mb-6 flex items-center gap-3 rounded-xl bg-zinc-50 p-4">
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-bold text-zinc-800">{video.title}</p>
            <p className="text-xs text-zinc-500">{video.duration} • {video.date}</p>
          </div>
        </div>
        ...
        <div className="mb-6 flex flex-col gap-1.5">
          <label htmlFor="confirm-name" className="text-sm font-semibold text-zinc-700">
            Ketik <span className="text-red-600">{video.title}</span> untuk konfirmasi
          </label>
          <input
            id="confirm-name"
            type="text"
