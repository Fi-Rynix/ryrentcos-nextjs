...
interface Article {
  ...
  title: string;
  image: string;
  category: "AniManga" | "IRL Event" | "Gorengan";
  createdAt: string;
}
...
export default function ArticleDeleteDialog({ article, onClose }: ArticleDeleteDialogProps) {
  const [isDeleting, setIsDeleting] = useState(false);
  const [confirmName, setConfirmName] = useState("");

  const handleDelete = async () => {
    if (confirmName !== article.title) return;
  ...
  const isConfirmValid = confirmName === article.title;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
        ...
        <div className="mb-6 text-center">
          <h2 className="text-xl font-bold text-zinc-800">Hapus Artikel?</h2>
        ...
        <div className="mb-6 flex items-center gap-3 rounded-xl bg-zinc-50 p-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-zinc-500">{article.createdAt}</p>
          </div>
        </div>
        ...
        <div className="mb-6 flex flex-col gap-1.5">
          <label htmlFor="confirm-name" className="text-sm font-semibold text-zinc-700">
            Ketik <span className="text-red-600">{article.title}</span> untuk konfirmasi
          </label>
          <input
            id="confirm-name"
            type="text"
