...
interface Product {
  ...
  images: string[];
  detailImage: string;
  badges: ("trending" | "brandNew" | "topSelling")[];
...
export default function ProductDeleteDialog({ product, onClose }: ProductDeleteDialogProps) {
  const [isDeleting, setIsDeleting] = useState(false);
  const [confirmName, setConfirmName] = useState("");

  const handleDelete = async () => {
    if (confirmName !== product.name) return;
  ...
  const isConfirmValid = confirmName === product.name;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
        ...
        <div className="mb-6 text-center">
          <h2 className="text-xl font-bold text-zinc-800">Hapus Produk?</h2>
        ...
        <div className="mb-6 flex items-center gap-3 rounded-xl bg-zinc-50 p-4">
          <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-lg bg-red-50">
            <Image
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate text-sm font-bold text-zinc-800">{product.name}</p>
            <p className="text-sm font-bold text-orange-800">{formatPrice(product.price)}</p>
            <p className="text-xs text-zinc-500 capitalize">Kategori: {product.category}</p>
          </div>
        </div>

        {/* Confirmation Input */}
        <div className="mb-6 flex flex-col gap-1.5">
          <label htmlFor="confirm-name" className="text-sm font-semibold text-zinc-700">
            Ketik <span className="text-red-600">{product.name}</span> untuk konfirmasi
          </label>
          <input
            id="confirm-name"
            type="text"
